...
const GetAnnotatedMessageSchema = z.object({
  ...
  includeImage: z
    .boolean()
    .default(false)
    .describe("Whether to include an image in the message"),
});
...
export const registerGetAnnotatedMessageTool = (server: McpServer) => {
  server.registerTool(
    ...
    async (args: any): Promise<CallToolResult> => {
      const { messageType, includeImage } =
        GetAnnotatedMessageSchema.parse(args);

      const content: CallToolResult["content"] = [];

      // Main message with different priorities/audiences base on type
