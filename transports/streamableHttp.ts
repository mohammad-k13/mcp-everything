...
app.get("/mcp", async (req: Request, res: Response) => {
  console.log("Recieved MCP GET request");
  const sessionId = req.headers["mcp-session-id"] as string | undefined;
  if (!sessionId || !transports.has(sessionId)) {
    res.status(400).json({
      jsonrpc: "2.0",
  ...
  // Check GET requests for SSe streams
...
// Hanlde DELETE requests for session cleanup
...
app.get("/mcp/transports", (_req, res) => {
  // List all current session IDs and possibly some lightweight session info
  const activeTransports = Array.from(transports.entries()).map(([sessionId, transport]) => ({
    sessionId,
    // For security, only return safe details. Add more fields as needed.
    isOpen: !!transport,
  }));
  res.status(200).json({
    jsonrpc: "2.0",
    result: activeTransports,
  });
});
